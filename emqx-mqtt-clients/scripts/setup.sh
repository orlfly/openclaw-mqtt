...
echo ""
echo "————————————————————————————"
echo "【1.5】MQTT 认证（可选，无认证留空）"
echo "————————————————————————————"
echo ""

current="${EMQX_MQTT_USERNAME:-}"
read -r -p "MQTT 用户名 [${current}]: " input
EMQX_MQTT_USERNAME="${input:-${current}}"

current="${EMQX_MQTT_PASSWORD:-}"
prompt="MQTT 密码"
if [ -n "$current" ]; then
  prompt="$prompt [**** (已设置)]"
fi
read -r -s -p "${prompt}: " input
echo ""
EMQX_MQTT_PASSWORD="${input:-${EMQX_MQTT_PASSWORD:-}}"
...
# MQTT 认证（可选）
EMQX_MQTT_USERNAME=${EMQX_MQTT_USERNAME}
EMQX_MQTT_PASSWORD=${EMQX_MQTT_PASSWORD}
...
export EMQX_MQTT_USERNAME EMQX_MQTT_PASSWORD
